<?php $this->assign('title', 'Danh sách dự án: ' . $user->fullname); ?>
<section class="content-header">
    <h1>
        Danh sách dự án của <b><i><?= $user->fullname ?></i></b>
    </h1>
    <ol class="breadcrumb">
        <li><?= $this->Html->link('<i class="fa fa fa-home"></i> Trang chủ', '/', ['escape' => false]) ?></li>
        <li class="active">Danh sách dự án của thành viên</li>
    </ol>
</section>
<section class="content">
    ...
    <div class="box">
        <div class="box-header">
            <h3 class="box-title">Danh sách dự án đã và đang tham gia</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body no-padding">
            <table class="table table-striped">
                ...
                <?php $current_page = $this->Paginator->params()['page'];
                ...
                <tr>
                    ...
                    <td>
                        <?=
                        $this->Html->link(
                        $this->Text->truncate($project['project']['name'], 30, [
                        'ellipsis' => '...',
                        'exact' => true,
                        'html' => false,
                        ]), [
                        'action' => 'viewProject', $project['project']['project_id']
                        ])
                        ?>
                    </td>
                    ...
                    <?php elseif ($project['project']['status'] == 0): ?>
                    <td class="text-center">Đã xóa</td>
                    <?php else : ?>
                    <td class="text-center">Unknown</td>
                    <?php endif; ?>
                    <?php if ($project['project']['start_date'] != NULL): ?>
                    <td class="text-center"><?= $project['project']['start_date']->format('Y-m-d') ?></td>
                    <?php else: ?>
                    <td class="text-center">NULL</td>
                    <?php endif; ?>
                    <?php if ($project['project']['end_date'] != NULL): ?>
                    <td class="text-center"><?= $project['project']['end_date']->format('Y-m-d') ?></td>
                    <?php else: ?>
                    <td class="text-center">-</td>
                    <?php endif; ?>
                    <td>
                        <?=
                        $this->Text->truncate($project['project']['description'], 50, [
                        'ellipsis' => '...',
                        'exact' => true,
                        'html' => false,
                        ])
                    ...
                    <td>
                        <?php
                        $arrLeader = [];
                        foreach ($project['project']['users'] as $leader){
                    ...
                    <?php if ($userLogin['role'] != 'MEMBERS'): ?>
                    <td><?= $this->Html->link('Sửa', ['action' => 'edit', $project['project']['project_id']]) ?>
                        | <?= $this->Form->postLink(
                        'Xóa', ['action' => 'deleteProject', $project['project']['project_id']], ['confirm' => 'Bạn muốn xóa?']
                        )
                        ?></td>
                    <?php endif; ?>
